...
- **Spring Boot**  
  Java 기반의 웹 애플리케이션 프레임워크로, 빠른 개발과 유지보수를 위해 선택했습니다. RESTful API 설계와 다양한 스프링 생태계 연동이 용이해 서비스 확장성과 안정성을 확보할 수 있습니다.
...
- **Amazon RDS (MySQL)**  
  데이터베이스 운영의 복잡성을 줄이고, 자동 백업 및 장애 복구 등 관리 편의성을 위해 AWS RDS를 사용합니다. 인프라 관리 부담을 줄이고, 서비스 신뢰성을 높입니다.

- **AWS EC2**  
  백엔드 서버와 애플리케이션을 안정적으로 배포하고 운영하기 위해 EC2 인스턴스를 활용합니다. 유연한 확장성과 AWS 인프라 연동이 용이합니다.
...
- **Jira**  
  프로젝트 관리와 이슈 추적을 체계적으로 하기 위해 Jira를 사용합니다. 스프린트 관리, 업무 분배, 진행 상황 파악에 효과적입니다.

- **Slack**  
  팀원 간 실시간 커뮤니케이션과 빠른 피드백을 위해 Slack을 활용합니다. 다양한 외부 서비스와 연동이 가능해 협업 효율을 높입니다.

- **GitHub**  
  소스 코드 버전 관리와 협업을 위해 GitHub를 사용합니다. 코드 리뷰, 이슈 관리, CI/CD 연동 등 개발 프로세스 전반을 지원합니다.
...
- **Spring Data JPA**  
  데이터베이스 접근을 추상화하고, 반복적인 CRUD 코드 작성을 줄이기 위해 Spring Data JPA를 사용합니다. 생산성과 일관성을 높일 수 있습니다.
